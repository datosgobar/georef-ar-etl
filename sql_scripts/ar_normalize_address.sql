-- Function: tiger.ar_normalize_address(character varying)

-- DROP FUNCTION tiger.ar_normalize_address(character varying);

CREATE OR REPLACE FUNCTION tiger.ar_normalize_address(in_rawinput character varying)
...
BEGIN
  ...
  -- Extrae los dígitos de la altura si estań presentes en la dirección.
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION tiger.ar_normalize_address(character varying)
  OWNER TO postgres;
COMMENT ON FUNCTION tiger.ar_normalize_address(character varying) IS 'args: in_address - Given a textual street address, returns a composite norm_addy type that has road suffix, prefix and type standardized, street, streetname etc. broken into separate fields. This function will work with just the lookup data packaged with the tiger_geocoder.';

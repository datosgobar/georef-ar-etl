...
def get_centroid_coordinates(geom, ctx):
    centroid_json = ctx.scalar(geom.ST_Centroid().ST_AsGeoJSON())
    centroid = json.loads(centroid_json)
    return centroid['coordinates']


def get_intersection_centroid(geom_a, geom_b, ctx):
    return ctx.scalar(geom_a.ST_Intersection(geom_b).ST_Centroid())
...
def get_intersection_percentage(geom_a, geom_b, ctx):
    ...
    area_a = ctx.scalar(geom_a.ST_Area())

    if area_a <= 0:
        raise ValueError('geom_a has area size 0')

    area_isct = ctx.scalar(geom_a.ST_Intersection(geom_b).ST_Area())

...
BEGIN
  ...
  THEN
    ...
    EXECUTE format(sql, 'name', colum_name, 'code', colum_code, 'update', colum_code,
                   tbl_name, tbl_name_tmp, colum_name, colum_name, colum_code, colum_code)
    INTO result;
  END IF;
...
BEGIN
  ...
  THEN
    ...
    EXECUTE format(sql, 'code', colum_code, 'name', colum_name, 'update', colum_name,
                   tbl_name, tbl_name_tmp, colum_code, colum_code, colum_name, colum_name)
    INTO result;
  END IF;
...
BEGIN
  ...
  THEN
    sql := 'SELECT json_build_object(%L, NULL, %L, count(*)) FROM %s';
    EXECUTE format(sql, 'entrada', 'guardada', tbl_name) INTO result;

  ELSE
    sql := 'SELECT json_build_object(%L, count(*), %L, NULL)' ||
           ' FROM %s';
    EXECUTE format(sql, 'entrada', 'guardada', tbl_name_tmp) INTO result;
  END IF;
  RETURN result;
END;
$$;
...
BEGIN
  ...
  THEN
    sql := 'SELECT json_agg(json_build_object(%L, ogc_fid, %L, %I))' ||
           ' FROM %s' ||
           ' WHERE %I ISNULL';
    EXECUTE format(sql, 'id', 'name', column_name, tbl_name, column_code)
    INTO result;
  END IF;
...
BEGIN
  ...
  THEN
    SELECT json_agg(json_build_object('code', in1, 'name', nam))
    INTO result
    FROM ign_provincias_tmp
    WHERE in1 NOT IN (
...
BEGIN
  ...
  THEN
    ...
    EXECUTE format(sql, 'code', column_code, 'name', column_name,
                   tbl_state, tbl_name, column_code, column_code)
    INTO result;
  END IF;
